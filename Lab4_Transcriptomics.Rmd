...
Gene expression was different in each group: mock, avirulent and virulent.

Arabidopsis responds to each attack, physical, bacterial and viral by differentially expressing its genes.

The groupings differ between the two approaches because the first approach uses the whole set of genes while the second approach only uses a subset of the genes, those showing differential expression, to perform the clustering.

The second approach better matches what we would expect. At a given point in time, mock samples should be grouped with mock samples, avirulent samples should be grouped with avirulent samples and virulent samples should be grouped with virulent samples.
